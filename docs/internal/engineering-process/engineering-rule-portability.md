...
Eleven entries have a source-reviewed `external-trial-active` mapping to exact
LED or `plesk_support` contracts at source revision
`d1c6f6d1af8442ce704cf57c07a28ec4ed6aec66`. `ENG-PORT-DEL-010` has reached
`cross-project-evidence-recorded` through source review `SR-DEL010-01`; no entry
has reached `extraction-review-ready` or a higher status. Other external
uses and local drafts must name the matching `ENG-PORT-*` rule revision in a
tracked owning-project record before promotion.
...
| ENG-PORT-DISC-002 | Discovery | Record facts as confirmed, provisional, unknown, or rejected; absence of evidence is not a default | KCS incident causality keeps unknown classifications explicit; LED contract `ETC-LED-DISC-002-r1` is active and collecting evidence | external-trial-active | Close the LED rule evidence with decision impact, corrections, effort, overhead, false positives, limitations, and a local verdict for source review |
| ENG-PORT-DISC-003 | Discovery | Complete the chain stakeholder answers -> workflow model -> data/domain constraints -> measurable baseline -> bounded hypothesis -> controlled experiment -> result/correction log | LED contract `ETC-LED-DISC-003-r1` is active against the one-sample controlled experiment; the physical result remains pending | external-trial-active | Close the LED evidence chain with the measured result, corrections, overhead, false positives, limitations, and local verdict |
...
| ENG-PORT-DISC-006 | Discovery | Before finalizing a design or starting substantial implementation that depends on an existing runtime/API, prove the exact endpoint, mode, response shape, and operating condition with fresh evidence or one bounded safe smoke; neighboring paths and fixtures are nominal evidence only | KCS-15.2b1 `/api/snippets` planning correction; `plesk_support` contract `ETC-PS-DISC-006-r1` is active against a blocked external-service capability path | external-trial-active | Close the `plesk_support` exact-path evidence with the next clarification/revalidation, decision impact, corrections, overhead, false positives, limitations, and local verdict |
| ENG-PORT-DES-001 | Design | Map each observable acceptance criterion to a deterministic gate, bounded model trial, or named human-review gate before implementation. For a branching or stateful workflow, enumerate and gate the supported outcome-by-context transition matrix; testing every outcome in isolation plus one happy-path sequence is insufficient. | KCS functional-test convention and policy anchors; LED contract `ETC-LED-DES-001-r1` is active with a predeclared physical acceptance gate and result branches | external-trial-active | Close LED evidence with the physical result, acceptance-map corrections, test/review burden, false confidence, limitations, and local verdict |
| ENG-PORT-DES-002 | Design | State contracts intended to change and contracts required to remain unchanged before a material slice | KCS slice plans, review packets, and freeze policy; `plesk_support` contract `ETC-PS-DES-002-r1` is active on a narrow privacy/egress and main-only integration boundary | external-trial-active | Close the `plesk_support` evidence with contract conflicts/drift prevented, corrections, review effort, false positives, limitations, and local verdict |
...
| ENG-PORT-DES-004 | Design | Assign deterministic algorithms, AI proposals, and human decisions to explicit owners; AI output remains untrusted until validated | KCS runtime architecture and packet validation enforce this locally; LED contract `ETC-LED-DES-004-r1` is active and collecting evidence | external-trial-active | Close the LED ownership evidence with bypass prevention, correction effort, false positives, human residuals, limitations, and a local verdict for source review |
...
| ENG-PORT-DES-007 | Design | Track outcome agreement, design selection, and Delivery authorization independently; no state implies another | KCS-15.1 follow-on transition exposed ambiguous continuation wording; LED contract `ETC-LED-DES-007-r1` is active across separate Design, Delivery, authentic-execution, and physical-manufacture gates | external-trial-active | Close LED evidence with prevented or attempted inherited approvals, corrections, operator burden, false-positive stops, limitations, and local verdict |
...
| ENG-PORT-DEL-006 | Delivery | Nondeterministic or model-mediated acceptance predeclares fixtures, trial count, conditions, thresholds, variance, and safe observability | KCS-14.5 closeout distinguishes deterministic Python proof from Desktop/model interpretation; LED contract `ETC-LED-DEL-006-r1` is active with the physical trial count, conditions, threshold, provenance, and interpretation frozen before measurement | external-trial-active | Close LED evidence with the physical observation, uncertainty handling, corrections, overhead, false positives, limitations, and local verdict |
...
| ENG-PORT-DEL-008 | Delivery | An isolated adapter, spike, prototype, or experiment proves only its bounded feasibility claim; parent UX selection and integration authorization remain separate and locked | KCS-15.2a proves a bounded local public RAG adapter while its closeout leaves KCS-15.2b design open and Delivery locked; LED contract `ETC-LED-DEL-008-r1` is active and collecting evidence | external-trial-active | Close the LED bounded-feasibility evidence with inherited-approval attempts, corrections, overhead, false positives, limitations, and a local verdict for source review |
